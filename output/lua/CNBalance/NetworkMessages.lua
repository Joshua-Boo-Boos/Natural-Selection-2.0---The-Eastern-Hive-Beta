...
-- "Take Credits" (CLIENT -> SERVER). Carries no payload: the exchange is a fixed size and the server
-- re-validates everything (CE active, holding a Combat Builder, team can afford it), so there is
-- nothing for a client to usefully lie about.
Shared.RegisterNetworkMessage("CETakeCredits", {})

-- Combat Engineers sound preference (CLIENT -> SERVER). The CE cues are played server-side with
-- Server.PlayPrivateSound, so a client-side checkbox cannot decline them locally - it has to tell
-- the server, which then skips that player when broadcasting.
Shared.RegisterNetworkMessage("CESoundPref", { enabled = "boolean" })

-- Combat Engineers team announcement (SERVER -> CLIENT).
--
-- Deliberately NOT the vanilla "Chat" message. That path always prefixes the line with "(Team) " or
-- "(All) " - see Chat.lua, `teamOnly and locationNameText or "(All) "` - with no way to suppress it,
-- so announcements came out as "(Team) : [Com. Eng.] ...". Routing these through the client's system
-- message channel instead gives the exact "[Com. Eng.]: ..." form with nothing prepended.
Shared.RegisterNetworkMessage("CETeamAnnounce", { message = "string (220)" })
