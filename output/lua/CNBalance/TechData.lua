...
function BuildTechData()
    ...
    -- "Take Credits" build-menu entry. Not a structure and never placed: it has no map name, model
    -- or build time on purpose, so nothing can try to spawn it. Only the display name and tooltip
    -- are needed, for the menu button that triggers the team->personal resource exchange.
    table.insert(techData,{
        [kTechDataId] = kTechId.CombatEngineersCredits,
        [kTechDataDisplayName] = "CE_TAKE_CREDITS",
        [kTechDataTooltipInfo] = "CE_TAKE_CREDITS_TOOLTIP",
        [kTechDataCostKey] = 0,
    })
