...
local kKillCommandDamageLockoutTime = 5
    ...
    if client ~= nil then
        ...
        if player ~= nil and (not client.timeLastKillCommand or client.timeLastKillCommand + 1 < Shared.GetTime()) then
            ...
            if HasMixin(player, "Live") and player:GetCanDie() then
                ...
                local timeOfLastDamage = player:GetTimeOfLastDamage()
                if timeOfLastDamage and Shared.GetTime() - timeOfLastDamage < kKillCommandDamageLockoutTime then
                    Server.SendNetworkMessage(player, "Chat", BuildChatMessage(false, "SERVER", -1, player:GetTeamNumber(), kNeutralTeamType, "You cannot use the Kill command within five seconds of taking damage."), true)
                else
