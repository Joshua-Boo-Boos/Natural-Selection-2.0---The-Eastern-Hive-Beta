...
local kCEStructures =
{
    ...
    -- TWO Arms Lab entries, one per upgrade track. Both place the SAME structure and keep
    -- kTechId.ArmsLab, so cost, supply, the six-lab cap and the tech-tree availability check all keep
    -- working untouched -- only the track differs, and it decides what the lab researches on its own.
    { techId = kTechId.ArmsLab, track = "weapon", label = "Arms Lab (W)" },
    { techId = kTechId.ArmsLab, track = "armor",  label = "Arms Lab (A)"   },
...
local function MakeAbility(def)
    ...
    -- Plain fields, read by CombatBuilder when the structure is created and by the build menu when
    -- it labels the button. NOT methods: the "never touch self" rule above applies to methods that
    -- the menu may call with a dot, and a field sidesteps the question entirely.
    ability.ceTrack = def.track

    -- Distinguishes the two Arms Lab buttons, which share a tech id and would otherwise both read
    -- "Arms Lab". Everything else falls back to the tech data display name.
    if def.label then
        function ability:GetMenuDisplayName() return def.label end
    end
    ...
    function ability:IsAllowed(player)
        ...
        -- SUPPLY. A CE marine places structures without a commander ever seeing them, so nothing
        -- else in this path enforces the team supply cap - it would simply be exceeded. Checked
        -- BEFORE placement (rather than letting the structure go down and counting it afterwards)
        -- so the button greys out and the drop is refused, instead of the team silently ending up
        -- over cap.
        --
        -- Both helpers are side-agnostic on purpose: GetSupplyUsedByTeam reads the team on the
        -- server and the networked TeamInfo entity on the client, so the greyed button and the
        -- server's refusal always agree.
        local supplyCost = LookupTechData(techId, kTechDataSupply, 0) or 0
        if supplyCost > 0 then

            local teamNumber = player:GetTeamNumber()
            local supplyUsed = GetSupplyUsedByTeam(teamNumber) or 0
            local supplyMax  = GetMaxSupplyForTeam(teamNumber) or 0

            if supplyUsed + supplyCost > supplyMax then
                return false
            end

        end

        -- def.track, not the tech id: the two Arms Lab buttons share kTechId.ArmsLab and are capped
        -- at three EACH, so only the track can say which of the two counters this button is against.
        return GetCombatEngineersCanBuild(techId, player:GetTeamNumber(), def.track)
    end
...
for _, def in ipairs(kCEStructures) do
    ...
    -- Keyed by tech id, EXCEPT where a tech id appears more than once (the two Arms Labs), which
    -- would otherwise silently overwrite the first entry. The ordered list below is what the menu
    -- actually uses -- CombatBuilder.kSupportedStructures is indexed, not keyed -- so the key only
    -- has to stay unique, not meaningful.
    local key = def.track and ( tostring(def.techId) .. "_" .. def.track ) or def.techId
    kCEStructureAbilities[key] = ability
...
-- ============================================================
-- "Take Credits" - a menu entry that is not a structure
-- ============================================================
-- Sits in the same list as the structures purely so it inherits the menu's paging, hotkeys, icon and
-- label handling for free. It is NOT placeable, and is marked so with ceIsCreditsExchange: every
-- placement path keys off that flag and refuses it, and selecting it in the menu fires the exchange
-- immediately instead of arming a ghost (see MarineBuild_SendSelect).
--
-- Appended AFTER CombatBuilder adds the Sentry and Supply Depot, so it takes the third slot on
-- page 3 alongside them rather than displacing anything.
kCECreditsAbility =
{
    ceIsCreditsExchange = true,
}

function kCECreditsAbility:GetDropStructureId()
    return kTechId.CombatEngineersCredits
end

function kCECreditsAbility:GetDropMapName()
    return nil
end

-- Never placeable: nothing may ever be spawned from this entry.
function kCECreditsAbility:GetRequiresOrientationClick()
    return false
end

function kCECreditsAbility:GetMaxStructures()
    return 0
end

function kCECreditsAbility:IsAllowed(player)

    if not player or not GetCombatEngineersActive(player) then
        return false
    end

    -- Greys out whenever nothing could actually be exchanged - an empty team pool, or a marine
    -- already sitting on the personal resource cap. Both terms are networked, so this reads the same
    -- on the client as the server's own check.
    return GetCombatEngineersCreditsExchangeAmount(player) > 0
end
