...
function MarineBuild_Close()
    ...
    if dropStructureAbility then
        ...
        -- Destroying the GUI object is NOT enough to keep it shut. CombatBuilder:OnUpdateRender
        -- recreates self.buildMenu on the very next render frame whenever it is nil, and it reads
        -- menuActive to decide whether to show it - so leaving menuActive true here means the menu
        -- pops straight back. That is what made a right-click appear to close it only for as long as
        -- the button was HELD: OverrideInput re-closed it every frame while the button was down, and
        -- the frame after release it simply came back.
        --
        -- Cleared here rather than in each caller so EVERY close path is correct by construction;
        -- MarineBuild_SendSelect already did this for the selection path and is now redundant but
        -- harmless.
...
function MarineBuild_SendSelect(index)
    ...
    if player then
        ...
        -- "Take Credits" is an ACTION, not a structure: it must never arm a placement ghost. Fire the
        -- exchange and close, leaving activeStructure untouched so the player keeps whatever
        -- structure they already had selected.
        local ability = CombatBuilder.kSupportedStructures[index]
        if ability and ability.ceIsCreditsExchange then

            Client.SendNetworkMessage("CETakeCredits", {}, true)

            -- Play the confirmation LOCALLY for the marine who clicked, rather than relying on the
            -- server broadcast to reach them. The button is greyed unless the team can afford the
            -- exchange, so by the time it is clickable the conversion is going to succeed.
            --
            -- This is the same client-side path the build-menu open sound uses, which is known to
            -- work; the server broadcast (below, in the CETakeCredits handler) still covers everyone
            -- else on the team, and skips the initiator so it is never heard twice.
            -- Once per life, and silent with the option off - both handled by the claim.
            if CombatEngineers_PlayLocalSoundOnce then
                CombatEngineers_PlayLocalSoundOnce(kCombatEngineersCreditsTakenSound)
            end
...
-- `track` is optional and only ever set for the two Arms Lab buttons, whose price ladders are
-- counted separately. Everything else ignores it.
function MarineBuild_GetStructureCost(techId, track)

    local player = Client.GetLocalPlayer()
    if not player then return 0 end

    return CombatBuilder.GetStructureCost(techId, player:GetTeamNumber(), nil, track)

end

-- The track a build-menu button serves, or nil for a button that has none.
function MarineBuild_GetTrackForIndex(index)
    local ability = index and CombatBuilder.kSupportedStructures[index]
    return ability and ability.ceTrack or nil
end
...
function MarineBuild_GetStructureCount(techId, track)
    ...
    local count, max = GetCombatEngineersStructureCount(techId, player:GetTeamNumber(), track)
    if count then
        return count, max
    end
...
local function UpdateButton(button, hovered)
    ...
    local buttonTrack = MarineBuild_GetTrackForIndex(button.index)
    local count, max = MarineBuild_GetStructureCount(techId, buttonTrack)
    ...
    local cost = MarineBuild_GetStructureCost(techId, buttonTrack)
    if cost == 0 then
...
-- Rewrites the "N t-res / N p-res" pair on the Take Credits button to the amount that would actually
-- be exchanged right now. Cheap enough to run every frame: one arithmetic helper and two SetText
-- calls, and only for the single button that has these labels.
function GUIMarineBuildMenu:UpdateCreditsAmountText(button)

    if not button or not button.creditsAmountText then
        return
    end

    local amount = 0

    if GetCombatEngineersCreditsExchangeAmount then
        amount = GetCombatEngineersCreditsExchangeAmount(Client.GetLocalPlayer()) or 0
    end

    button.creditsAmountText[1]:SetText(string.format("%d t-res", amount))
    button.creditsAmountText[2]:SetText(string.format("%d p-res", amount))
...
function GUIMarineBuildMenu:Update(deltaTime)
    ...
    for _, button in ipairs(self.buttons) do
        ...
        self:UpdateCreditsAmountText(button)
...
function GUIMarineBuildMenu:CreateButton(techId, index, scale, frame, keybind, column)
    ...
    -- "Take Credits" is the one entry that is an exchange rather than a structure, and its icon alone
    -- does not say what the exchange IS. Three short lines are drawn OVER the icon spelling out the
    -- trade, in marine blue. Deliberately semi-transparent so the icon still reads through it - this
    -- annotates the button rather than replacing its artwork.
    if techId == kTechId.CombatEngineersCredits then

        -- "Take Credits" is the one entry that is an exchange rather than a structure, and its icon
        -- alone does not say what the exchange IS. Laid out to read as a flow down through the icon:
        --
        --        20 t-res
        --      v  [icon]  v
        --        20 p-res
        --
        -- The two arrows sit either side of the icon so they frame it rather than cover it, and the
        -- whole annotation is semi-transparent so the artwork still reads through.
        local creditsColor = Color(GUIMarineBuildMenu.kAvailableColor.r,
                                   GUIMarineBuildMenu.kAvailableColor.g,
                                   GUIMarineBuildMenu.kAvailableColor.b,
                                   0.97)

        -- The two rows are NOT symmetric: the top label sits closer to centre than the bottom one so
        -- it reads as tucked under the icon's upper edge rather than floating off it.
        local kCreditsTopOffset    = 26  -- "20 t-res", above centre
        local kCreditsBottomOffset = 36  -- "20 p-res", below centre
        local kCreditsArrowOffset = 34   -- how far left/right of centre the two arrows sit

        button.creditsLines = {}

        local function MakeCreditsText(text, offsetX, offsetY, fontSize)

            local item = self:CreateAnimatedTextItem()
            item:SetUniformScale(scale)
            item:SetText(text)
            item:SetFontName(kFontName)
            item:SetFontSize(fontSize)
            item:SetFontIsBold(true)
            item:SetColor(creditsColor)
            ...
            item:SetPosition(Vector(offsetX, offsetY, 0))

            button.graphicItem:AddChild(item)

            table.insert(self.pageAnimatedItems, item)
            table.insert(button.creditsLines, item)

            -- RETURNED so the caller can keep a handle on it. Without this,
            -- `creditsAmountText = { MakeCreditsText(...), MakeCreditsText(...) }` built a table of
            -- two nils, and the per-frame refresh then indexed nil the moment the menu opened.
            return item

        end

        -- Seeded with the live amount and REFRESHED EVERY FRAME from Update, because the value moves
        -- while the menu is open: personal resources tick up, teammates spend from the pool, and a
        -- marine can cross the cap without touching anything. A number captured once at page-build
        -- time would quietly go stale and promise resources the exchange would not deliver.
        --
        -- The true figure is shown even when it is 0 - that is what explains why the button is
        -- greyed. It can never exceed 20; the helper takes the smallest of the chunk, the room under
        -- the cap, and the team pool.
        button.creditsAmountText = {
            MakeCreditsText("", 0, -kCreditsTopOffset,    18),
            MakeCreditsText("", 0,  kCreditsBottomOffset, 18),
        }

        self:UpdateCreditsAmountText(button)

        -- Plain "v" rather than a unicode arrow glyph: the rest of this menu is ASCII (the label used
        -- to be "->") and a missing glyph would render as a box.
        MakeCreditsText("v", -kCreditsArrowOffset, 0, 22)
        MakeCreditsText("v",  kCreditsArrowOffset, 0, 22)

    end

    button.description:SetUniformScale(scale)
    -- Prefer a label supplied by the ability itself. The two Arms Lab buttons share kTechId.ArmsLab,
    -- so the tech-data display name alone would print "Arms Lab" on both and leave the player no way
    -- to tell the weapons one from the armour one.
    local menuAbility = CombatBuilder.kSupportedStructures[index]
    local labelText = menuAbility and menuAbility.GetMenuDisplayName and menuAbility:GetMenuDisplayName()
                      or Locale.ResolveString(LookupTechData(techId, kTechDataDisplayName, ""))

    button.description:SetText(labelText)
...
function GUIMarineBuildMenu:OverrideInput(input)
    ...
    -- RIGHT-CLICK ANYWHERE CLOSES. Left-clicking outside a button already closes the chooser (the
    -- click falls through untouched to CombatBuilder:OnPrimaryAttack, whose toggle flips it shut),
    -- so this covers the other button and gives an unmissable way out either way.
    --
    -- The command is CONSUMED, so it never reaches CombatBuilder:OnSecondaryAttack - otherwise the
    -- same click would close this chooser and immediately open the structure upgrade window behind
    -- it, which are explicitly meant to be mutually exclusive.
    if HasMoveCommand( input.commands, Move.SecondaryAttack ) then

        input.commands = RemoveMoveCommand( input.commands, Move.SecondaryAttack )
